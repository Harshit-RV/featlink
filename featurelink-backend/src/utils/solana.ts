...
export const mintFeatCoinToAddress = async (address: string, amount: number): Promise<boolean> => {
  tokenTransfer2(new PublicKey(address), amount);
  return true;
}

const tokenTransfer2 = async (publicKey : PublicKey, amount: number) => {
